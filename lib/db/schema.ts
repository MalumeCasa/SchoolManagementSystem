...
export const classes = pgTable("classes", {
	...
	teachers: text().array().default([""]).notNull(),
...
export const registeredStudents = pgTable("registered_students", {
	...
	unique("registered_students_email_key").on(table.email),
	unique("registered_students_phone_key").on(table.phone),
...
export const students = pgTable("students", {
	...
	foreignKey({
			...
			name: "students_registered_student_id_fkey"
		}).onDelete("set null"),
...
export const teacherClasses = pgTable("teacher_classes", {
	...
	foreignKey({
			...
			name: "teacher_classes_teacher_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
...
export const subjects = pgTable("subjects", {
	...
	teacherIds: integer("teacher_ids").array().default([]).notNull(),
	teacherNames: text("teacher_names").array().default([""]).notNull(),
...
export const subjectClasses = pgTable("subject_classes", {
	...
	foreignKey({
			...
			name: "subject_classes_subject_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "subject_classes_class_id_fkey"
		}).onDelete("cascade"),
...
export const subjectTeachers = pgTable("subject_teachers", {
	...
	foreignKey({
			columns: [table.subjectId],
			foreignColumns: [subjects.id],
			name: "subject_teachers_subject_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.teacherId],
			foreignColumns: [teachers.id],
			name: "subject_teachers_teacher_id_fkey"
		}).onDelete("cascade"),
	unique("unique_subject_teacher").on(table.subjectId, table.teacherId),
]);
...
export const classPeriods = pgTable("class_periods", {
	...
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "class_periods_class_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.subjectId],
			foreignColumns: [subjects.id],
			name: "class_periods_subject_id_fkey"
		}).onDelete("set null"),
	unique("unique_class_period").on(table.classId, table.periodNumber, table.dayOfWeek),
	check("class_periods_day_of_week_check", sql`(day_of_week >= 0) AND (day_of_week <= 6)`),
	check("check_time_range", sql`start_time < end_time`),
]);
...
export const curriculum = pgTable("curriculum", {
	...
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "curriculum_class_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.subjectId],
			foreignColumns: [subjects.id],
			name: "curriculum_subject_id_fkey"
		}).onDelete("cascade"),
...
export const curriculumChapters = pgTable("curriculum_chapters", {
	...
	index("idx_curriculum_chapters_curriculum").using("btree", table.curriculumId.asc().nullsLast().op("int4_ops")),
	index("idx_curriculum_chapters_order").using("btree", table.curriculumId.asc().nullsLast().op("int4_ops"), table.chapterOrder.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.curriculumId],
			foreignColumns: [curriculum.id],
			name: "curriculum_chapters_curriculum_id_fkey"
		}).onDelete("cascade"),
	unique("unique_chapter_curriculum_order").on(table.curriculumId, table.chapterOrder),
]);
...
export const curriculumTopics = pgTable("curriculum_topics", {
	...
	index("idx_curriculum_topics_chapter").using("btree", table.chapterId.asc().nullsLast().op("int4_ops")),
	index("idx_curriculum_topics_order").using("btree", table.chapterId.asc().nullsLast().op("int4_ops"), table.topicOrder.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.chapterId],
			foreignColumns: [curriculumChapters.id],
			name: "curriculum_topics_chapter_id_fkey"
		}).onDelete("cascade"),
	unique("unique_topic_chapter_order").on(table.chapterId, table.topicOrder),
]);
...
export const curriculumProgress = pgTable("curriculum_progress", {
	...
	foreignKey({
			columns: [table.curriculumId],
			foreignColumns: [curriculum.id],
			name: "curriculum_progress_curriculum_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "curriculum_progress_class_id_fkey"
		}).onDelete("cascade"),
...
export const studentCurriculumProgress = pgTable("student_curriculum_progress", {
	...
	foreignKey({
			columns: [table.studentId],
			foreignColumns: [students.id],
			name: "student_curriculum_progress_student_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.curriculumId],
			foreignColumns: [curriculum.id],
			name: "student_curriculum_progress_curriculum_id_fkey"
		}).onDelete("cascade"),
...
export const assignments = pgTable("assignments", {
	...
	index("idx_assignments_teacher").using("btree", table.teacherId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
...
export const grades = pgTable("grades", {
	...
	foreignKey({
			...
			name: "grades_exam_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.recordedBy],
			foreignColumns: [staff.id],
			name: "grades_recorded_by_fkey"
		}).onDelete("set null"),
...
export const assignmentSubmissions = pgTable("assignment_submissions", {
	...
	index("idx_assignment_submissions_assignment").using("btree", table.assignmentId.asc().nullsLast().op("int4_ops")),
	index("idx_assignment_submissions_status").using("btree", table.status.asc().nullsLast().op("text_ops")),
	index("idx_assignment_submissions_student").using("btree", table.studentId.asc().nullsLast().op("int4_ops")),
	foreignKey({
	...
	foreignKey({
			...
			foreignColumns: [teachers.id],
			name: "assignment_submissions_graded_by_fkey"
		}).onDelete("set null"),
...
export const exams = pgTable("exams", {
	...
	index("idx_exams_subject").using("btree", table.subjectId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
...
export const feeStructure = pgTable("fee_structure", {
	...
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "fee_structure_class_id_fkey"
		}).onDelete("set null"),
]);
...
export const studentFees = pgTable("student_fees", {
	...
	foreignKey({
			columns: [table.studentId],
			foreignColumns: [students.id],
			name: "student_fees_student_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.feeStructureId],
			foreignColumns: [feeStructure.id],
			name: "student_fees_fee_structure_id_fkey"
		}).onDelete("cascade"),
]);
...
export const staffLeave = pgTable("staff_leave", {
	...
	index("idx_staff_leave_dates").using("btree", table.startDate.asc().nullsLast().op("date_ops"), table.endDate.asc().nullsLast().op("date_ops")),
	index("idx_staff_leave_staff").using("btree", table.staffId.asc().nullsLast().op("int4_ops")),
	index("idx_staff_leave_status").using("btree", table.status.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.staffId],
			foreignColumns: [staff.id],
			name: "staff_leave_staff_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.approvedBy],
			foreignColumns: [staff.id],
			name: "staff_leave_approved_by_fkey"
		}).onDelete("set null"),
	check("check_leave_dates", sql`start_date <= end_date`),
]);
...
export const feePayments = pgTable("fee_payments", {
	...
	foreignKey({
			...
			name: "fee_payments_student_fee_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.receivedBy],
			foreignColumns: [staff.id],
			name: "fee_payments_received_by_fkey"
		}).onDelete("set null"),
]);
...
export const feeDiscounts = pgTable("fee_discounts", {
	...
	foreignKey({
			columns: [table.studentId],
			foreignColumns: [students.id],
			name: "fee_discounts_student_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.feeStructureId],
			foreignColumns: [feeStructure.id],
			name: "fee_discounts_fee_structure_id_fkey"
		}).onDelete("set null"),
]);
...
export const studentAttendance = pgTable("student_attendance", {
	...
	index("idx_student_attendance_class_date").using("btree", table.classId.asc().nullsLast().op("date_ops"), table.date.asc().nullsLast().op("date_ops")),
	...
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "student_attendance_class_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.subjectId],
			foreignColumns: [subjects.id],
			name: "student_attendance_subject_id_fkey"
		}).onDelete("set null"),
	foreignKey({
			columns: [table.recordedBy],
			foreignColumns: [staff.id],
			name: "student_attendance_recorded_by_fkey"
		}).onDelete("set null"),
	unique("unique_student_attendance").on(table.studentId, table.date, table.subjectId),
	check("attendance_status_check", sql`(status)::text = ANY (ARRAY['present'::text, 'absent'::text, 'late'::text, 'half-day'::text])`),
]);
...
export const attendanceSummary = pgTable("attendance_summary", {
	...
	foreignKey({
			columns: [table.studentId],
			foreignColumns: [students.id],
			name: "attendance_summary_student_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "attendance_summary_class_id_fkey"
		}).onDelete("cascade"),
	unique("unique_attendance_summary").on(table.studentId, table.month, table.year),
	check("attendance_summary_month_check", sql`(month >= 1) AND (month <= 12)`),
]);
...
export const staffAttendance = pgTable("staff_attendance", {
	...
	index("idx_staff_attendance_date").using("btree", table.date.asc().nullsLast().op("date_ops")),
	index("idx_staff_attendance_staff").using("btree", table.staffId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.staffId],
			foreignColumns: [staff.id],
			name: "staff_attendance_staff_id_fkey"
		}).onDelete("cascade"),
	unique("unique_staff_attendance").on(table.staffId, table.date),
]);
...
export const staffSalary = pgTable("staff_salary", {
	...
	index("idx_staff_salary_effective").using("btree", table.effectiveDate.asc().nullsLast().op("date_ops")),
	index("idx_staff_salary_staff").using("btree", table.staffId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.staffId],
			foreignColumns: [staff.id],
			name: "staff_salary_staff_id_fkey"
		}).onDelete("cascade"),
]);
...
export const reportCards = pgTable("report_cards", {
	...
	foreignKey({
			columns: [table.studentId],
			foreignColumns: [students.id],
			name: "report_cards_student_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.academicYearId],
			foreignColumns: [academicYears.id],
			name: "report_cards_academic_year_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.termId],
			foreignColumns: [academicTerms.id],
			name: "report_cards_term_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.classId],
			foreignColumns: [classes.id],
			name: "report_cards_class_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
...
export const chatRoomMembers = pgTable("chat_room_members", {
	...
	index("idx_chat_room_members_room").using("btree", table.roomId.asc().nullsLast().op("int4_ops")),
	index("idx_chat_room_members_user").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.userType.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const termConfig = pgTable("term_config", {
	...
	index("idx_term_config_dates").using("btree", table.startDate.asc().nullsLast().op("date_ops"), table.endDate.asc().nullsLast().op("date_ops")),
...
export const archivedStudents = pgTable("archived_students", {
	id: integer(),
	registeredStudentId: integer("registered_student_id"),
	studentId: varchar("student_id", { length: 50 }),
	name: text(),
...
export const archivedRegisteredStudents = pgTable("archived_registered_students", {
	id: integer(),
	studentId: text("student_id"),
	surname: text(),
	name: text(),
	...
	numberOfChildrenInFamily: integer("number_of_children_in_family"),
	positionInFamily: integer("position_in_family"),
	authorizedToBring: text("authorized_to_bring"),
	authorizedToCollect: text("authorized_to_collect"),
	previousSchool: text("previous_school"),
	intendedPrimarySchool: text("intended_primary_school"),
	careRequired: text("care_required"),
	...
	specialInstructions: text("special_instructions"),
	medicalConsent1: text("medical_consent_1"),
	medicalConsent1Father: boolean("medical_consent_1_father"),
	medicalConsent1Mother: boolean("medical_consent_1_mother"),
	medicalConsent1Guardian: boolean("medical_consent_1_guardian"),
	medicalConsent2: text("medical_consent_2"),
	medicalConsent2Father: boolean("medical_consent_2_father"),
	medicalConsent2Mother: boolean("medical_consent_2_mother"),
	medicalConsent2Guardian: boolean("medical_consent_2_guardian"),
	motherTitle: text("mother_title"),
	motherSurname: text("mother_surname"),
	motherFirstNames: text("mother_first_names"),
	motherIdNumber: text("mother_id_number"),
	...
	monthlyAmount: integer("monthly_amount"),
	paymentDate: integer("payment_date"),
	popiConsent: boolean("popi_consent"),
	motherPopiSignature: text("mother_popi_signature"),
	motherPopiDate: text("mother_popi_date"),
	fatherPopiSignature: text("father_popi_signature"),
	fatherPopiDate: text("father_popi_date"),
	...
	witnessName: text("witness_name"),
	witnessSignature: text("witness_signature"),
	indemnityAgreement: boolean("indemnity_agreement"),
	familyDoctor: text("family_doctor"),
	doctorPhone: text("doctor_phone"),
...
}).as(sql`SELECT sa.date, c.id AS class_id, c.class_name, count(DISTINCT sa.student_id) AS total_students, count( CASE WHEN sa.status::text = 'present'::text THEN 1 ELSE NULL::integer END) AS present_count, count( CASE WHEN sa.status::text = 'absent'::text THEN 1 ELSE NULL::integer END) AS absent_count, count( CASE WHEN sa.status::text = 'late'::text THEN 1 ELSE NULL::integer END) AS late_count, count( CASE WHEN sa.status::text = 'half-day'::text THEN 1 ELSE NULL::integer END) AS half_day_count, round(count( CASE WHEN sa.status::text = 'present'::text THEN 1 ELSE NULL::integer END)::numeric * 100.0 / NULLIF(count(DISTINCT sa.student_id), 0)::numeric, 2) AS attendance_percentage FROM student_attendance sa JOIN classes c ON sa.class_id = c.id GROUP BY sa.date, c.id, c.class_name`);

export const studentPerformanceView = pgView("student_performance_view", {	studentId: integer("student_id"),
	name: text(),
	surname: text(),
	className: text("class_name"),
	classSection: text("class_section"),
...
}).as(sql`SELECT s.id AS student_id, s.name, s.surname, s.class_name, s.class_section, rc.total_percentage, rc.overall_grade, rc.position_in_class, rc.attendance_percentage, ay.name AS academic_year, at.name AS term, rc.generated_at FROM report_cards rc JOIN students s ON s.id = rc.student_id JOIN academic_years ay ON ay.id = rc.academic_year_id JOIN academic_terms at ON at.id = rc.term_id WHERE rc.is_published = true`);

export const teacherWorkloadView = pgView("teacher_workload_view", {	teacherId: integer("teacher_id"),
	name: text(),
	surname: text(),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	totalClasses: bigint("total_classes", { mode: "number" }),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	totalSubjects: bigint("total_subjects", { mode: "number" }),
	assignments: jsonb(),
}).as(sql`SELECT t.id AS teacher_id, t.name, t.surname, count(DISTINCT tc.class_id) AS total_classes, count(DISTINCT st.subject_id) AS total_subjects, jsonb_agg(DISTINCT jsonb_build_object('class', (c.class_name || ' '::text) || c.class_section, 'subject', s.name, 'is_primary', tc.is_primary)) AS assignments FROM teachers t LEFT JOIN teacher_classes tc ON tc.teacher_id = t.id LEFT JOIN classes c ON c.id = tc.class_id LEFT JOIN subject_teachers st ON st.teacher_id = t.id LEFT JOIN subjects s ON s.id = st.subject_id GROUP BY t.id, t.name, t.surname`);

export const classPerformanceSummary = pgView("class_performance_summary", {	classId: integer("class_id"),
	className: text("class_name"),
	classSection: text("class_section"),
	// You can use { mode: "bigint" } if numbers are exceeding js number limitations
	totalStudents: bigint("total_students", { mode: "number" }),
	avgPerformance: numeric("avg_performance"),
	maxPerformance: numeric("max_performance"),
	minPerformance: numeric("min_performance"),
	avgAttendance: numeric("avg_attendance"),
}).as(sql`SELECT c.id AS class_id, c.class_name, c.class_section, count(DISTINCT s.id) AS total_students, round(avg(rc.total_percentage), 2) AS avg_performance, max(rc.total_percentage) AS max_performance, min(rc.total_percentage) AS min_performance, round(avg(attendance_summary.present_days::numeric / NULLIF(attendance_summary.total_school_days, 0)::numeric * 100::numeric), 2) AS avg_attendance FROM classes c LEFT JOIN students s ON s.class_id = c.id LEFT JOIN report_cards rc ON rc.class_id = c.id AND rc.is_published = true LEFT JOIN attendance_summary ON attendance_summary.class_id = c.id GROUP BY c.id, c.class_name, c.class_section`);

export const feeArrearsView = pgView("fee_arrears_view", {	studentId: integer("student_id"),
	name: text(),
	surname: text(),
	className: text("class_name"),
	classSection: text("class_section"),
